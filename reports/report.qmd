...
select
  `Region`,
  round(100.0 * sum(case when `Income Group` = 'High income' then country_count end) / g.total_count, 2) as "High income",
  round(100.0 * sum(case when `Income Group` = 'Upper middle income' then country_count end) / g.total_count, 2) as "Upper middle income",
  round(100.0 * sum(case when `Income Group` = 'Lower middle income' then country_count end) / g.total_count, 2) as "Lower middle income",
  round(100.0 * sum(case when `Income Group` = 'Low income' then country_count end) / g.total_count, 2) as "Low income",
  round(100.0 * sum(case when `Income Group` is null then country_count end) / g.total_count, 2) as "Other",
  round(100.0 * sum(country_count) / g.total_count, 2) as "Row Total %"
from
  base, grand_total g
group by
  `Region`, g.total_count
order by
  `Region`
""")
...
select
  ...
  i.country_count,
  t.total_count,
  round(100.0 * i.country_count / t.total_count, 2) as percent_of_total
from
  income_counts i,
  total t
order by
